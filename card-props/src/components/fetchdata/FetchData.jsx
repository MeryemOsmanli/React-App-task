...
function FetchData() {
   ...
   async function getAllInformation(params) {
        ...
        <div>
            ...
            <div>
                <form onSubmit={handleSubmit}>
                    ...
                    <br />
                    <input type="text" value={priceInp} onChange={(e)=>setPriceInp(e.target.value)}/>
                    <br />
                    <button >Add</button>

                </form>
            {information.map((x) => (
          <div className='card_box' key={x.id}  style={{ color: x.discontinued ? "green" : "red" }}>
            <h3>{x.name}</h3>
            <p>{`${x.unitPrice} $`}</p>
